...
import net.ymate.wechat.model.WechatMessage;
...
public class MessageEventListener implements IEventListener<WechatEvent> {
    ...
    public boolean handle(WechatEvent context) {
        ...
        if (_content != null) {
            __doSaveMessage(context.getSource().getMsgId(), context.getSource().getFromUserName(), context.getSource().getToUserName(), context.getSource().getMsgType().ordinal(), _content, context.getSource().getCreateTime());
        }
        return false;
    }
    ...
    private void __doSaveMessage(final String msgId,
        ...
        try {
            String _id = DigestUtils.md5Hex(msgId + toUserName + createTime);
            if (WechatMessage.builder().id(_id).build().load(Fields.create(WechatMessage.FIELDS.ID)) == null) {
                WechatMessage.builder().id(_id)
